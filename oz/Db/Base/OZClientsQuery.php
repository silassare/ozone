<?php
	...
	abstract class OZClientsQuery
	{
		...
		public function find($max = null, $offset = 0, array $order_by = [])
		{
			...
			return new \OZONE\OZ\Db\OZClientsResults($this->db, $this->resetQuery());
		}
		...
		/**
		 * Filters rows with condition on column `create_time` in the table `oz_clients`.
		 *
		 * @param mixed  $value    the filter value
		 * @param int    $operator the operator to use
		 *
		 * @return $this|\OZONE\OZ\Db\OZClientsQuery
		 */
		public function filterByCreateTime($value, $operator = Rule::OP_EQ)
		{
		    return $this->filterBy('create_time', $value, $operator);
		}
