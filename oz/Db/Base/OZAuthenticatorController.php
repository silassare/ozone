 ...
 */
	...
	use OZONE\OZ\Db\OZAuthenticatorResults as OZAuthenticatorResultsReal;
	...
	abstract class OZAuthenticatorController
	{
		...
		public function __construct()
		{
			...
			$table    = $this->db->getTable(OZAuth::TABLE_NAME);
		 ...
		 *
		 * @param \OZONE\OZ\Db\Base\OZAuthenticatorQuery $query
		 * @param array                               $filters
		 *
		 * @throws \Gobl\ORM\Exceptions\ORMControllerFormException
		 * @throws \Gobl\ORM\Exceptions\ORMException
		...
		/**
		 * Update all items in `oz_authenticator` that match the given item filters.
		 *
		 * @param array $filters    the row filters
		 * @param array $new_values the new values
		 *
		...
		public function updateAllItems(array $filters, array $new_values)
		{
			...
			$tableQuery = new OZAuthenticatorQueryReal();
		...
		public function getAllItemsCustom(QueryBuilder $qb, $max = null, $offset = 0, &$total = false)
		{
			...
			$results = new OZAuthenticatorResultsReal($this->db, $qb);
		...
		/**
		 * @param \OZONE\OZ\Db\OZAuthenticatorResults $results
		 * @param int                         $found
		 * @param null|int                    $max
		 * @param int                         $offset
		 *
		 ...
		 * @throws \Gobl\DBAL\Exceptions\DBALException
		 */
		private static function totalResultsCount(OZAuthenticatorResultsReal $results, $found = 0, $max = null, $offset = 0)
		{
			$total = 0;
			if ($total !== false) {
				if (isset($max)) {
