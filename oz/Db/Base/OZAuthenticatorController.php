 ...
 */
	...
	abstract class OZAuthenticatorController extends ORMControllerBase
	{
		...
		{
			parent::__construct(ORM::getDatabase('OZONE\OZ\Db'), OZAuth::TABLE_NAME, OZAuthReal::class, OZAuthenticatorQueryReal::class, OZAuthenticatorResultsReal::class);
		}
		...
		/**
		 * Updates all items in `oz_authenticator` that match the given item filters.
		 *
		 * @param array $filters    the row filters
		 * @param array $new_values the new values
		 *
		...
		/**
		 * Deletes one item from `oz_authenticator`.
		 *
		 * The returned value will be:
		 * - `false` when the item was not found
		...
		/**
		 * Deletes all items in `oz_authenticator` that match the given item filters.
		 *
		 * @param array $filters the row filters
		 *
		 * @return int Affected row count.
		...
		{
			/** @var \OZONE\OZ\Db\OZAuth[] $results */
