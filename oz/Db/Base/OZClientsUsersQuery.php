<?php
	...
	/**
	 * Class OZClientsUsersQuery
	 *
	 * @package OZONE\OZ\Db\Base
	 */
	abstract class OZClientsUsersQuery
	{
		/** @var \Gobl\DBAL\Table */
		protected $table;
		/** @var string */
		...
		/**
		 * OZClientsUsersQuery constructor.
		 */
		...
		/**
		 * Finds rows in the table `oz_clients_users`.
		 *
		...
		public function find($max = null, $offset = 0)
		{
			...
			return new \OZONE\OZ\Db\OZClientsUsersResults($this->db, $this, $this->resetQuery());
		}

		/**
		 * Delete rows in the table `oz_clients_users`.
		 *
		 * When filters exists only rows that
		...
		/**
		 * Update rows in the table `oz_clients_users`.
		 *
		 * When filters exists only rows that
		...
		/**
		 * Safely update rows in the table `oz_clients_users`.
		 *
		...
		/**
		 * Filters rows with condition on column `client_api_key` in the table `oz_clients_users`.
		 *
		 * @param mixed  $value    the filter value
		 * @param int    $operator the operator to use
		 *
		...
		/**
		 * Filters rows with condition on column `user_id` in the table `oz_clients_users`.
		 *
		 * @param mixed  $value    the filter value
		 * @param int    $operator the operator to use
		 *
		...
		/**
		 * Filters rows with condition on column `session_id` in the table `oz_clients_users`.
		 *
		 * @param mixed  $value    the filter value
		 * @param int    $operator the operator to use
		 *
		 * @return $this|\OZONE\OZ\Db\OZClientsUsersQuery
		 */
		public function filterBySessionId($value, $operator = Rule::OP_EQ)
		{
		    return $this->filterBy('session_id', $value, $operator);
		}

		/**
		 * Filters rows with condition on column `token` in the table `oz_clients_users`.
		 *
		 * @param mixed  $value    the filter value
		 * @param int    $operator the operator to use
		 *
		...
		/**
		 * Filters rows with condition on column `last_check` in the table `oz_clients_users`.
		 *
		 * @param mixed  $value    the filter value
		 * @param int    $operator the operator to use
		 *
		 * @return $this|\OZONE\OZ\Db\OZClientsUsersQuery
		 */
		public function filterByLastCheck($value, $operator = Rule::OP_EQ)
		{
		    return $this->filterBy('last_check', $value, $operator);
		}
