<?php
	 ...
	 */
	abstract class OZCountriesQuery
	{
		/** @var \Gobl\DBAL\Table */
		protected $table;
		/** @var string */
		...
		public function __construct()
		{
			...
			$this->table       = $this->db->getTable('oz_countries');
			$this->table_alias = $this->getUniqueAlias();
			$this->qb          = new QueryBuilder($this->db);
		}

		/**
		 * Finds rows in the table `oz_countries`.
		 *
		...
		public function find($max = null, $offset = 0)
		{
			...
			return new \OZONE\OZ\Db\OZCountriesResults($this->db, $this, $this->resetQuery());
		}

		/**
		 * Delete rows in the table `oz_countries`.
		 *
		 * When filters exists only rows that
		...
		/**
		 * Update rows in the table `oz_countries`.
		 *
		 * When filters exists only rows that
		...
		/**
		 * Safely update rows in the table `oz_countries`.
		 *
		...
		public function addOrUpdate(array $row)
		{
			...
			$entity = new \OZONE\OZ\Db\OZCountry($is_new);
			$entity->hydrate($row);
		...
		/**
		 * Filters rows in the table `oz_countries`.
		 *
