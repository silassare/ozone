 ...
 */
	 ...
	 */
	abstract class OZSessionsController
	{
		/**
		 * @var array
		 */
		...
		public function __construct()
		{
			...
			$table    = $this->db->getTable(OZSession::TABLE_NAME);
		 ...
		 * the column(s) to update really exists in `oz_sessions`.
		 *
		 ...
		 *
		 * @param \OZONE\OZ\Db\Base\OZSessionsQuery $query
		 * @param array                               $filters
		 *
		 * @throws \Gobl\ORM\Exceptions\ORMControllerFormException
		 * @throws \Gobl\ORM\Exceptions\ORMException
		 * @throws \Exception
		 */
		final protected static function applyFilters(OZSessionsQuery &$query, array $filters)
		{
			if (empty($filters)) {
				return;
			}
		...
		/**
		 * Update all items in `oz_sessions` that match the given item filters.
		 *
		 * @param array $filters    the row filters
		 * @param array $new_values the new values
		 *
		...
		public function updateAllItems(array $filters, array $new_values)
		{
			...
			$tableQuery = new OZSessionsQueryReal();
		...
		/**
		 * Delete one item from `oz_sessions`.
		 *
		 * The returned value will be:
		 * - `false` when the item was not found
		...
		/**
		 * Delete all items in `oz_sessions` that match the given item filters.
		 *
		 * @param array $filters the row filters
		 *
		 * @return int Affected row count.
		...
		public function getAllItemsCustom(QueryBuilder $qb, $max = null, $offset = 0, &$total = false)
		{
			...
			$results = new OZSessionsResultsReal($this->db, $qb);
		...
		/**
		 * @param \OZONE\OZ\Db\OZSessionsResults $results
		 * @param int                         $found
		 * @param null|int                    $max
		 * @param int                         $offset
		 *
		 ...
		 * @throws \Gobl\DBAL\Exceptions\DBALException
		 */
		private static function totalResultsCount(OZSessionsResultsReal $results, $found = 0, $max = null, $offset = 0)
		{
			$total = 0;
			if ($total !== false) {
				if (isset($max)) {
		...
		/**
		 * Gets collection items from `oz_sessions`.
		 *
		 * @param string   $name
		 * @param array    $filters
		 * @param null|int $max
		...
		/**
		 * Find all items in `oz_sessions` that match the given filters.
		 *
		 * @param array    $filters  the row filters
