<?php
	...
	use Gobl\ORM\ORM;
	use OZONE\OZ\Db\OZClientUser as OZClientUserReal;
	use OZONE\OZ\Db\OZClientsUsersQuery as OZClientsUsersQueryReal;

	/**
	 * Class OZClientsUsersController
	 *
	 * @package OZONE\OZ\Db\Base
	 */
	abstract class OZClientsUsersController
	{
		/** @var array */
		...
		/**
		 * OZClientsUsersController constructor.
		 */
		 ...
		 * the column(s) to update really exists in `oz_clients_users`.
		 *
		 ...
		 *
		 * @param \OZONE\OZ\Db\Base\OZClientsUsersQuery $query
		 * @param array                               $item_filters
		 *
		 * @throws \Gobl\ORM\Exceptions\ORMControllerFormException
		 */
		final protected static function applyFilters(OZClientsUsersQuery &$query, array $item_filters)
		{
			if (empty($item_filters)) {
				return;
			}
		...
		public function addItem(array $form = [])
		{
			...
			$my_entity = new OZClientUserReal();
		...
		/**
		 * Updates one item in `oz_clients_users`.
		 *
		 * The returned value will be:
		 * - `false` when the item was not found
		 * - `OZClientUser` when the item was successfully updated,
		 ...
		 *
		 * @return bool|\OZONE\OZ\Db\OZClientUser
		 */
		...
		/**
		 * Delete one item from `oz_clients_users`.
		 *
		 * The returned value will be:
		 * - `false` when the item was not found
		 * - `OZClientUser` when the item was successfully deleted,
		...
		public function deleteOneItem(array $item_filters)
		{
			...
			if ($my_entity) {
				$my_query = new OZClientsUsersQueryReal();
		...
		/**
		 * Delete all items in `oz_clients_users` that match the given item filters.
		 *
		 * @param array $item_filters the item filters
		 *
		...
		/**
		 * Gets item from `oz_clients_users` that match the given filters.
		 *
		 * The returned value will be:
		 * - `null` when the item was not found
		 * - `OZClientUser` otherwise
		 *
		 * @param array $item_filters
		 *
		 * @return \OZONE\OZ\Db\OZClientUser|null
		 */
		...
		/**
		 * Gets all items from `oz_clients_users` that match the given filters.
		 *
		 * @param array $item_filters
		 * @param int   $max
		 * @param int   $offset
		...
		/**
		 * Find all items in `oz_clients_users` that match the given filters.
		 *
		 * @param array $item_filters
		 *
		 * @param int   $max
		 * @param int   $offset
		 *
		 * @return \OZONE\OZ\Db\OZClientsUsersResults
		 */
