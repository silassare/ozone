 ...
 */
	...
	abstract class OZClientsController
	{
		...
		public function __construct()
		{
			...
			$table    = $this->db->getTable(OZClient::TABLE_NAME);
		...
		public function updateAllItems(array $filters, array $new_values)
		{
			...
			$tableQuery = new OZClientsQueryReal();
		...
		public function getAllItemsCustom(QueryBuilder $qb, $max = null, $offset = 0, &$total = false)
		{
			...
			$results = new OZClientsResultsReal($this->db, $qb);
		...
		/**
		 * @param \OZONE\OZ\Db\OZClientsResults $results
		 * @param int                         $found
		 * @param null|int                    $max
		 * @param int                         $offset
		 *
		 ...
		 * @throws \Gobl\DBAL\Exceptions\DBALException
		 */
		private static function totalResultsCount(OZClientsResultsReal $results, $found = 0, $max = null, $offset = 0)
		{
			$total = 0;
			if ($total !== false) {
				if (isset($max)) {
		...
		/**
		 * Gets collection items from `oz_clients`.
		 *
		 * @param string   $name
		 * @param array    $filters
		 * @param null|int $max
