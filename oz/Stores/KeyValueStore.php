 ...
 * Class KeyValueStore.
 *
 * The API of both a cache store and a state store: `CacheRegistry` and `StateRegistry` hand back
 * this same object, because the difference between them is the promise, not the operations. It is
 * not named after either one for that reason.
 *
 * The primary interface for components interacting with the cache.
 * Wraps a {@see StoreDriverInterface} driver with a convenient high-level API.
 *
 ...
 *   - `CacheRegistry::store('oz:form:sessions')` — named store from `oz.stores.cache` settings
 */
final class KeyValueStore
{
	 ...
	 * @param StoreDriverInterface $provider the underlying driver
	 */
	public function __construct(
		private readonly string $name,
		private readonly StoreDriverInterface $provider,
	) {}
	 ...
	 */
	public function getProvider(): StoreDriverInterface
	{
	 ...
	 */
	public function entry(string $key): ?StoreEntry
	{
	...
	public function set(string $key, mixed $value, ?int $ttl = null): bool
	{
		...
		return $this->provider->set(new StoreEntry($key, $value, $expires_at));
	}
