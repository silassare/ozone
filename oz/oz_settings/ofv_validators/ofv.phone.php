<?php
	...
	function ofv_phone(OFormValidator $ofv)
	{
		...
		} elseif (in_array('not-registered', $rules) AND UsersManager::searchUserWithPhone($phone)) {
			$ofv->addError('OZ_FIELD_PHONE_ALREADY_REGISTERED', ['phone' => $phone]);
		} elseif (in_array('registered', $rules) AND !UsersManager::searchUserWithPhone($phone)) {
