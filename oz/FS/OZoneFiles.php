<?php
	...
	final class OZoneFiles {
		...
		function __construct( $uid ) {
			...
			$this->user_files_dir = OZ_APP_USERS_FILES_DIR . $uid;
			$this->user_files_thumb_dir = OZ_APP_USERS_FILES_DIR . $uid . DS . 'thumb';

			//create this user files directory if not done
			OZoneFileSystem::mkdir( $this->user_files_dir );
			OZoneFileSystem::mkdir( $this->user_files_thumb_dir );
		}
		...
		private function isUploadedFileAlias( $file ) {
			...
			if ( !preg_match( "#^[a-z0-9_]+\.ofa$#i", $file[ 'name' ] ) )
				return false;
			//the correct mime type
			if ( $file[ 'type' ] !== "text/x-ozone-file-alias" )
				return false;
		...
		private function parseFileAlias( $alias_src ) {
			...
			move_uploaded_file( $alias_src, $this->user_files_dir . DS . $data[ 'fid' ] . ".ofa" );
		...
		public function genFileInfo( $source_name, $source_type ) {
			...
			$name_full = $name . '.' . $ext;
			$dest = $this->user_files_dir . DS . $name_full;
			$thumb = $this->user_files_thumb_dir . DS . $name . '.jpg';
		...
		public function logFile( $label ) {
			...
			if ( empty( $this->oz_file_info[ 'fid' ] ) ) {
				...
				$insert = OZoneDb::getInstance()->insert( $sql, array(
					'fid'    => null,
