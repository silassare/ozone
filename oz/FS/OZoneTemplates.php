<?php
	...
	final class OZoneTemplates
	{
		 ...
		 * @throws \OZONE\OZ\Exceptions\OZoneInternalError    when template file does not exists.
		 */
		public static function localize($template)
		{
			$no_prefix = OZoneStr::removePrefix($template, 'oz:');

			if (!($no_prefix === $template)) {
				return OZonePath::resolve(OZ_OZONE_DIR . 'oz_templates', $no_prefix);
			}

			if (!OZonePath::isRelative($template)) {
				return $template;
			}
			...
			throw new OZoneInternalError('OZ_TEMPLATE_FILE_NOT_FOUND', [$template]);
		}
	}
