<?php
	...
	final class OZoneLang
	{
		 ...
		 *
		 * Should be called first and once, before any other call to this class methode
		 *
		 * @return void
		 */
		public static function init()
		{
			if (!self::$ready) {
				...
				self::$user_lang    = OZoneSessions::get('oz_lang:prefered');

				// add template plugin
				// usage: @oz_lang( $key [, $data [, $lang ] ] )
				\OTpl::addPluginAlias('oz_lang', ['OZONE\OZ\Lang\OZoneLang', 'translate']);
			}
		}

		/**
		 * get availables languages list or check if a given one is defined
		 *
		 * @param string $key
		 *
		 * @return array | null
		...
		{
			return OZoneSettings::get('lang/lang.list', $key);
		}

		/**
		 * get enabled languages list from availables languages
		 *
		 * @return array
		 */
		...
		/**
		 * get language to use as user language
		 *
		 * @return String
		 */
		...
		public static function setUserLanguage($lang)
		{
			...
			if (isset($l[$lang])) {
				OZoneSessions::set('oz_lang:prefered', $lang);
			}
		}

		/**
		 * check if we have a valid lang key
		 *
		...
		public static function translate($key, $data = [], $lang = null)
		{
			...
			$l = OZoneSettings::get('lang/oz.' . $lang, $key);
		...
		/**
		 * get the language group of a given language
		 * ex: 'fr-bj' -> 'fr'
		 *
		 * @param string $lang
		 *
