<?php
	...
	class PolyglotUtils
	{
		...
		private static function getLangText($lang_key, $lang_code, $history = [])
		{
			...
			if (is_string($text) AND preg_match(self::PORTION_COPY_REG, $text)) {
				$in                 = [];
			...
			return is_null($text) ? $lang_key : $text;
		}
		...
		public static function parseText($lang_key, $data, $lang_code)
		{
			...
			if (is_string($text) AND preg_match(self::SIMPLE_REPLACE_REG, $text)) {
				$in = [];
		...
		public static function parseBrowserLanguage()
		{
			...
			if (count($browser_languages) AND count($enabled_languages)) {
				...
				foreach ($browser_languages as $lang => $q) {
					...
					if ($lang_group != $lang AND isset($enabled_languages[$lang_group])) {
						$advice = $lang_group;
						break;
					}
				}
