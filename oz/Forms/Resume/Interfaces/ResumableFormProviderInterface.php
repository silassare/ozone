...
namespace OZONE\Core\Forms\Resume\Interfaces;
...
interface ResumableFormProviderInterface
{
	 ...
	 * {@see FormResumeRouteInterceptor} and cannot be used via the standalone
	 * {@see ResumableFormService} endpoints.
	 *
	 * When true, {@see ResumableFormService} will reject any direct request to those
	 * endpoints with a {@see BadRequestException}. Default: true. Sessions of such a
	 * provider are always bound to the route they were opened on.
	 */
	 ...
	 * `$cleaned_fd` for the first {@see self::nextStep()} call.
	 *
	 ...
	 * auth state, etc. Called once per session action, by {@see FormSessionManager}.
	 *
	 ...
	 *
	 * - `$cleaned_fd`: accumulated validated fields from all previous steps
	 ...
	 *
	 * @param FormDataClean      $cleaned_fd accumulated validated fields from all previous steps
	 ...
	 * abandonment. The `$session` array is the raw cached record at the time of
	 * expiry: `provider_class`, `phase`, `cleaned_fd`, `progress_state`, `scope_id`,
	 * `scope_name`, `created_at`, `expires_at`, `history`, plus `provider_name`
	 * (standalone sessions) or `route` (route-bound sessions).
	 *
