<?php
	...
	class Route
	{
		 ...
		 * @param string|null $original_route
		 *
		 * @return string
		 */
		...
		{
			$len            = strlen($route);
			$original_route = is_null($original_route) ? $route : $original_route;
			$reg            = '';
			$pos            = 0;
		...
		private static function hasCatchingGroupOrIsInvalid($regexp)
		{
			...
			return
				false !== strpos($regexp, '(')
				OR false !== strpos($regexp, ')')
				OR false !== strpos($regexp, '?')
				OR false !== strpos($regexp, ':')
				OR 0 === strpos($regexp, '^') /* starts with ^ */
				OR strlen($regexp) - 1 === strpos($regexp, '$') /* ends with $ */
				;
		}
	}