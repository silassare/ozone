...
/**
 * Class FormResumeExpiredException.
 *
 * Thrown when a resumable form session has passed its hard deadline.
 *
 * HTTP 410 Gone — the session existed but is permanently no longer available.
 */
class FormResumeExpiredException extends BaseException
{
	/**
	 * FormResumeExpiredException constructor.
	 *
	...
	{
		parent::__construct(
			empty($message) ? 'OZ_FORM_RESUME_EXPIRED' : $message,
			$data,
			$previous,
			BaseException::GONE,
		);
