<?php
	...
	class WideImage_Operation_AutoCrop
	{
		...
		function execute($img, $margin, $rgb_threshold, $pixel_cutoff, $base_color)
		{
			...
			if ($rgb_threshold < 0) $rgb_threshold = 0;

			$pixel_cutoff = intval($pixel_cutoff);
			if ($pixel_cutoff <= 1) $pixel_cutoff = 1;

			if ($base_color === null) $rgb_base = $img->getRGBAt(0, 0); else {
				if ($base_color < 0) return $img->copy();
			...
			if ($cut_rect['left'] < 0) $cut_rect['left'] = 0;

			if ($cut_rect['top'] < 0) $cut_rect['top'] = 0;

			if ($cut_rect['right'] >= $img->getWidth()) $cut_rect['right'] = $img->getWidth() - 1;

			if ($cut_rect['bottom'] >= $img->getHeight()) $cut_rect['bottom'] = $img->getHeight() - 1;
