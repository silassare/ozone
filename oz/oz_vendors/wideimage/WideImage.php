<?php
	...
	class WideImage
	{
		...
		{
			if (self::$path === null) self::$path = dirname(__FILE__) . DIRECTORY_SEPARATOR;
		...
		{
			if (!extension_loaded('gd')) throw new WideImage_Exception("WideImage requires the GD extension, but it's apparently not loaded.");
		}
		...
		static function load($source)
		{
			...
			if ($source == '') $predictedSourceType = 'String';

			// Creating image via a valid resource
			if (!$predictedSourceType && self::isValidImageHandle($source)) $predictedSourceType = 'Handle';
			...
			if (!$predictedSourceType) {
				...
				for ($i = 0; $i < $maxlen; $i++) if (ord($source[$i]) < 32) {
			...
			if (isset($_FILES[$source]) && isset($_FILES[$source]['tmp_name'])) $predictedSourceType = 'Upload';

			// Otherwise, must be a file or an URL
			if (!$predictedSourceType) $predictedSourceType = 'File';
		...
		static function loadFromFile($uri)
		{
			...
			if (!self::isValidImageHandle($handle)) {
				try {
					...
					if ($mapper) $handle = $mapper->load($uri);
				...
				if (!self::isValidImageHandle($handle)) {
					...
					foreach ($custom_mappers as $mime_type => $mapper_class) {
						...
						if (self::isValidImageHandle($handle)) break;
					}
				}
			}

			if (!self::isValidImageHandle($handle)) throw new WideImage_InvalidImageSourceException("File '{$uri}' appears to be an invalid image source.");
		...
		{
			if (strlen($string) < 128) throw new WideImage_InvalidImageSourceException("String doesn't contain image data.");
			...
			if (!self::isValidImageHandle($handle)) throw new WideImage_InvalidImageSourceException("String doesn't contain valid image data.");
		...
		{
			if (!self::isValidImageHandle($handle)) throw new WideImage_InvalidImageSourceException("Handle is not a valid GD image resource.");

			if (imageistruecolor($handle)) return new WideImage_TrueColorImage($handle); else
		...
		{
			if (!array_key_exists($field_name, $_FILES)) throw new WideImage_InvalidImageSourceException("Upload field '{$field_name}' doesn't exist.");

			if (is_array($_FILES[$field_name]['tmp_name'])) {
				if (isset($_FILES[$field_name]['tmp_name'][$index])) $filename = $_FILES[$field_name]['tmp_name'][$index]; else {
					$result = [];
					foreach ($_FILES[$field_name]['tmp_name'] as $idx => $tmp_name) $result[$idx] = self::loadFromFile($tmp_name);
			...
			if (!file_exists($filename)) throw new WideImage_InvalidImageSourceException("Uploaded file doesn't exist.");
		...
		{
			if (!self::isValidImageHandle($handle)) throw new WideImage_InvalidImageHandleException("{$handle} is not a valid image handle.");
		}
	}
