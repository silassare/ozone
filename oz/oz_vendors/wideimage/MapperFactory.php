<?php
	...
	abstract class WideImage_MapperFactory
	{
		...
		static function selectMapper($uri, $format = null)
		{
			...
			if (array_key_exists($format, self::$mappers)) return self::$mappers[$format];
			...
			if (!class_exists($mapperClassName, false)) {
				...
				if (file_exists($mapperFileName)) require_once $mapperFileName;
			}
		...
		{
			if ($format == null) $format = self::extractExtension($uri);

			// mime-type match
			if (preg_match('~[a-z]*/[a-z-]*~i', $format)) if (isset(self::$mimeTable[strtolower($format)])) {
