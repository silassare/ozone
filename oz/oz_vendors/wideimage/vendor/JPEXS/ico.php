<?php
	 ...
	 *
	 *        Version changes:
	 *                v2.1 - redesigned sourcecode, phpdoc included, all internal functions and global variables have
	 *                prefix "jpexs_" v2.0 - For icons with Alpha channel now you can set background color
	 ...
	 *      - if you developing closesource application, you should add my name at least to "about" page of your web
	 *      application
	 *      - if you create an amazing modification, please contact me... I can publish link to your webpage if you're
	 *      interested...
	 *      - if you want to use my script in commercial application for earning money, you should make a donation to
	 *      me first
	 */
	 ...
	 * @param int    $icoColorCount Icon color count (For multiple icons ico file) - 2,16,256, ICO_TRUE_COLOR,
	 *                              ICO_XP_COLOR or ICO_MAX_COLOR
	...
	function imageCreateFromIco($filename, $icoColorCount = 16, $icoSize = 16, $alphaBgR = 255, $alphaBgG = 255, $alphaBgB = 255)
	{
		...
		for ($p = 0; $p < count($Ikona); $p++) {
			if (($ColMax == -1) or ($Ikona[$p]["NumberOfColors"] >= $Ikona[$ColMax]["NumberOfColors"])) if (($icoSize == $Ikona[$p]["Width"]) or ($icoSize == ICO_MAX_SIZE)) {
				$ColMax = $p;
			};

			if (($SizeMax == -1) or ($Ikona[$p]["Width"] >= $Ikona[$SizeMax]["Width"])) if (($icoColorCount == $Ikona[$p]["NumberOfColors"]) or ($icoColorCount == ICO_MAX_COLOR)) {
				$SizeMax = $p;
			};

			if ($Ikona[$p]["NumberOfColors"] == $icoColorCount) if ($Ikona[$p]["Width"] == $icoSize) {
				$IconID = $p;
			};
		};
		...
		if ($Ikona[$IconID]["BitCount"] >= 24) {
			...
			for ($y = 0; $y < $Ikona[$IconID]["Height"]; $y++) for ($x = 0; $x < $Ikona[$IconID]["Width"]; $x++) {
			...
			for ($p = 0; $p < count($Ikona[$IconID]["Paleta"]); $p++) $Paleta[$p] = imagecolorallocate($img, $Ikona[$IconID]["Paleta"][$p]["r"], $Ikona[$IconID]["Paleta"][$p]["g"], $Ikona[$IconID]["Paleta"][$p]["b"]);
		...
		$IsTransparent = false;
		for ($y = 0; $y < $Ikona[$IconID]["Height"]; $y++) for ($x = 0; $x < $Ikona[$IconID]["Width"]; $x++) if ($Ikona[$IconID]["Maska"][$x][$y] == 1) {
			$IsTransparent = true;
			break;
		};
		if ($Ikona[$IconID]["BitCount"] == 32) {
	...
	function jpexs_readIcon($filename, $id, &$Ikona)
	{
		...
		if ($Ikona[$p]["Info"]["BitsPerPixel"] <= 8) {
			...
			for ($y = $Height - 1; $y >= 0; $y--) {
				...
				for ($g = 0; $g < $Zbytek; $g++) jpexs_freadbyte($f);
			};
			...
			for ($y = $Height - 1; $y >= 0; $y--) {
				...
				for ($z = 0; $z < $Zbytek; $z++) jpexs_freadbyte($f);
			};
	 ...
	 *
	 * @param string    $filename                Target exefile
	 * @param string    $icoFileNameOrPath       Filename to save ico or path (Default "") Path if you want more than 1
	 *                                           icon. If "", the filename is "$icoIndex.ico"
	 * @param int|array $iconIndex               Index(es) of the icon in exefile  (Default -1) If -1, all icons are
	 *                                           saved, Can be an array of indexes.
	 *
				...
				for ($r = 0; $r < count($Ikona); $r++):
					if (($iconIndex == $r) or ($iconIndex == -1) or ((is_array($iconIndex)) and (in_array($r, $iconIndex)))):
						...
						if ($icoFileNameOrPath == "") {
							...
							if (($iconIndex == -1) or (is_array($iconIndex))) $savefile = $icoFileNameOrPath . "$r.ico";
						};
						$f2 = fopen($savefile, "w");
	 ...
	 * @param string         $filename Target ico file to save icon to, If ommited or "", image is written to snadard
	 *                                 output - use header("Content-type: image/x-icon");
	 */
			...
			if ($BitCount <= 8) {
				for ($y = $Height - 1; $y >= 0; $y--) {
					...
					for ($x = 0; $x < $Width; $x++) {
						...
						if ($color == $Transparent) $color = imagecolorexact($img, 0, 0, 0);
						if ($color == -1) $color = 0;
					...
					for ($z = 0; $z < $Zbytek; $z++) $ret .= jpexs_inttobyte(0);
				};
			};
			...
			if ($BitCount >= 24) {
				for ($y = $Height - 1; $y >= 0; $y--) {
					for ($x = 0; $x < $Width; $x++) {
						...
						$ret   .= jpexs_inttobyte($color["green"]);
						$ret   .= jpexs_inttobyte($color["red"]);
						if ($BitCount == 32) $ret .= jpexs_inttobyte(0);//Alpha for ICO_XP_COLORS
					};
					for ($z = 0; $z < $Zbytek; $z++) $ret .= jpexs_inttobyte(0);
				};
			};
