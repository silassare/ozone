<?php
	 ...
	 *
	 *        Version changes:
	 *                v1.1 - redesigned sourcecode, phpdoc included, all internal functions and global variables have
	 *                prefix "jpexs_"
	 *
	 * TODO list:
	...
	function imagebmp($img, $file = "", $useRLE = false)
	{
		...
		if ($BitCount <= 8) {
			for ($y = $Height - 1; $y >= 0; $y--) {
				...
				for ($z = 0; $z < $Zbytek; $z++) $retd .= jpexs_inttobyte(0);
			};
		};

		if (($useRLE) and ($BitCount == 8)) {
			for ($t = 0; $t < strlen($retd); $t += 4) {
				if ($t != 0) if (($t) % $Width == 0) $ret .= chr(0) . chr(0);
			...
			if ($RLECompression == 1) // $BI_RLE8
			{
				...
				while (true) {
					...
					while (!(($prefix == 0) and ($suffix == 0))) {
						...
						if ($prefix > 0) {
							$pocet = $prefix;
							for ($r = 0; $r < $pocet; $r++) $Data .= chr($suffix);
						};
						$prefix = jpexs_freadbyte($f);
						$suffix = jpexs_freadbyte($f);
						$pocetb += 2;
				...
				while (true) {
					...
					while (!(($prefix == 0) and ($suffix == 0))) {
						if ($prefix == 0) {
							...
							$jpexs_CurrentBit = 0;
							for ($h = 0; $h < $pocet; $h++) $Data .= chr(jpexs_freadbits($f, 4));
