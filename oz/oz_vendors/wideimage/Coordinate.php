<?php
	...
	{
		static protected $coord_align   = ["left", "center", "right", "top", "middle", "bottom"];
		static protected $coord_numeric = ["[0-9]+", "[0-9]+\.[0-9]+", "[0-9]+%", "[0-9]+\.[0-9]+%"];
		...
		static function parse($c)
		{
			...
			for ($i = 0; $i < $expr_len; $i++) {
				...
				if (in_array($char, $operators)) {
					...
					if ($i == $expr_len - 1) $flush_operand = true;
				}

				if ($flush_operand) {
					if (trim($current_operand) != '') $tokens[] = [
						'type'  => 'operand',
		...
		static function evaluate($coord, $dim, $sec_dim = null)
		{
			...
			if (preg_match("/^([+-])?({$comp_regex})$/", $coord, $matches)) {
				...
				} elseif (substr($val, -1) === '%') return intval(round($sign * $dim * floatval(str_replace('%', '', $val)) / 100));
				else
		...
		static function fix($value, $dim, $sec_dim = null)
		{
			...
			if (count($coord_tokens) == 0 || $coord_tokens[count($coord_tokens) - 1]['type'] != 'operand') throw new WideImage_InvalidCoordinateException("Couldn't parse coordinate '$value' properly.");
			...
			foreach ($coord_tokens as $token) {
				if ($token['type'] == 'operand') {
					...
					if ($operation == 1) $value = $value + $operand_value; elseif ($operation == -1) $value = $value - $operand_value;
					else
					...
					if ($token['value'] == '-') {
						if ($operation == 0) $operation = -1; else
