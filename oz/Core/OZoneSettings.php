<?php
	...
	final class OZoneSettings
	{
		 ...
		 *
		 * @return mixed
		 * @throws \OZONE\OZ\Exceptions\OZoneInternalError    when a setting group or required setting key is not
		 *                                                    defined.
		 */
		public static function get($setting_group_name, $key = null, $required = false)
		{
			...
			throw new OZoneInternalError('OZ_SETTINGS_UNDEFINED', [$setting_group_name, $key]);
		}
		...
		public static function set($setting_group_name, array $data, $overwrite = false)
		{
			...
			$fs = new OZoneFS('.');
			$fs->cd($parts['dirname'], true)
			   ->wf($parts['basename'], OZoneTemplates::compute('oz:gen/settings.info.otpl', $inject));
		...
		/**
		 * check a setting group name validity.
		 *
		 * @param string $setting_group_name the setting group name.
		 *
